...
contract IVNFarmX10 is Ownable {
    ...
    //IVNy token to check for investors
    iIVNyToken IVNy;
    
    //fundFactories to check for Managers
    ...
    constructor(
        ...
        totalAllocPoint = 1000;
         IVNy = iIVNyToken(_IVNyToken);
    }
    
    function initialize(IERC20 _lp, uint256 _amount) onlyOwner public {
    ...
    function addFactory(address _factory) onlyOwner external {
    ...
    function deposit(uint256 _amount) public {
        ...
        require(specialAddress == true, "Neither a Manager nor an Investor");
        PoolInfo storage pool = poolInfo[0];
    ...
    function burnRemainingTokens() external onlyOwner{ 
        require(block.number >= bonusEndBlock.add(648000), "Can't burn yet"); // Can only burn after 3 months(= 972000 blocks)
