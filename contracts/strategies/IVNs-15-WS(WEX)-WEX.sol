...
contract InvestinStrategyToken is ERC20("WS(WEX)-WEX", "IVNs-15"), Ownable, ReentrancyGuard, iStrategy {//3213k 3172k 
    ...
    //iExchangePair constant lPToken = iExchangePair(0xA34337690711CE3F265f56Ebd545Dda00d7C0405);
  ...
  constructor(address[] memory _fundFactories) public {
        ...
        IERC20(rewardToken).approve(address(rewardPool), MAX_UINT_VALUE);
    ...
    function enter(uint256 amount, bool isFund) external override nonReentrant{
        ...
        // //if(_baseToken!=_token0){
            ...
            path_BaseToToken[1] = rewardToken;
            // ( , uint resT1, ) = lPToken.getReserves();
            // uint swapAmount = calcOptimalSwapAmt(resT1, amount);
            // token1Amount = amount.sub(swapAmount);
            amount = exchangeRouter.swapExactTokensForTokens(amount, 0, path_BaseToToken, address(this), block.timestamp)[1];
        ...
        rewardPool.deposit(pid, amount, false);
        principalStaked = principalStaked.add(amount);
        
        if( _totalSupply > 0 ){
            amount = (amount.mul(multiplier)).div(1e18);
        }
    ...
    function exitHelper(uint256 mv) internal{
        ...
        // if(_baseToken != _token0){
            ...
            path_TokentoBase[0] = rewardToken;
            path_TokentoBase[1] = baseToken;
            mv = exchangeRouter.swapExactTokensForTokens(mv, 1, path_TokentoBase, address(this), block.timestamp)[1];
        // }
        // if(_baseToken != _token1){
        ...
        // }
        IERC20(baseToken).transfer(msg.sender, mv);
    }
    ...
    function reinvest() public nonReentrant{
        ...
        // (, , uint liquidity) = exchangeRouter.addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, address(this), block.timestamp);
        // require(liquidity>0, "ALF");
        rewardPool.deposit(pid, reward, false);
        principalStaked = principalStaked.add(reward);
    }
    ...
    function getStrategyTokenPrice() public view override returns (uint) {
        ...
        // }
        address[] memory path_TokentoStableCoin = new address[](3);
        path_TokentoStableCoin[0] = rewardToken;
        path_TokentoStableCoin[1] = baseToken;
        path_TokentoStableCoin[2] = stableCoin;
        ...
        // uint price = exchangeRouter.getAmountsOut(1e18, path_TokentoStableCoin)[1]; 
        ...
        uint _totalSupply = totalSupply();
        if(_totalSupply!=0)
            return (exchangeRouter.getAmountsOut(principalStaked, path_TokentoStableCoin)[2]).mul(1e18).div(_totalSupply);
        else
            return (exchangeRouter.getAmountsOut(1e18, path_TokentoStableCoin)[2]);
    }
