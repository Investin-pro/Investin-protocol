...
contract InvestinStrategyToken is ERC20("WS(USDT-WEX)-WEX", "IVNs-16"), Ownable, ReentrancyGuard, iStrategy {//3213k 3172k 
    ...
    iExchangeRouter constant exchangeRouter = iExchangeRouter(0xD48745E39BbED146eEC15b79cBF964884F9877c2);
    iExchangePair constant lPToken = iExchangePair(0x50e8D9Aa83eBDe9608074eC1faaDfD2E792D9B81);
    address constant token0 = 0x55d398326f99059fF775485246999027B3197955;
    address constant token1 = 0xa9c41A46a6B3531d28d5c32F6633dd2fF05dFB90;
    iWexMaster constant rewardPool = iWexMaster(0x22fB2663C7ca71Adc2cc99481C77Aaf21E152e2D);
    uint256 constant pid = 29;
    address constant rewardToken = 0xa9c41A46a6B3531d28d5c32F6633dd2fF05dFB90;
  ...
  constructor(address[] memory _fundFactories) public {
        ...
        IERC20(token1).approve(address(exchangeRouter), MAX_UINT_VALUE);
        lPToken.approve(address(rewardPool), MAX_UINT_VALUE);
        //IERC20(rewardToken).approve(address(exchangeRouter), MAX_UINT_VALUE); rewardToken == token1
    ...
    function viewPendingRewards() external view returns(uint rewardAmt){
        rewardAmt = rewardPool.pendingWex(pid, address(this));
    }
    ...
    function enter(uint256 amount, bool isFund) external override nonReentrant{
        ...
        // //if(_baseToken!=_token0){
            //Swap all BaseToken balance to USDT and then swap an optimal amount of USDT to WEX
            address[] memory path_BaseToToken = new address[](2); // path_BaseToToken0 here token1 == basetoken
            path_BaseToToken[0] = baseToken;
            path_BaseToToken[1] = token0;
            uint token0Amount = exchangeRouter.swapExactTokensForTokens(amount, 0, path_BaseToToken, address(this), block.timestamp)[1];
            
            path_BaseToToken[0] = token0;
            path_BaseToToken[1] = token1;
            (uint resT0, , ) = lPToken.getReserves();
            uint swapAmount = calcOptimalSwapAmt(resT0, token0Amount);
            token0Amount = token0Amount.sub(swapAmount);
            uint token1Amount = exchangeRouter.swapExactTokensForTokens(swapAmount, 0, path_BaseToToken, address(this), block.timestamp)[1];
        ...
        rewardPool.deposit(pid, liquidity, false);
        principalStaked = principalStaked.add(liquidity);
        
        if( _totalSupply > 0 ){
            amount = (liquidity.mul(multiplier)).div(1e18);
        }
    ...
    function exitHelper(uint256 mv) internal{
        ...
        // if(_baseToken != _token1){
            //Convert all WEX to USDT first
            address[] memory path_TokentoBase = new address[](2);
            path_TokentoBase[0] = token1;
            path_TokentoBase[1] = token0;
            token1Amount = exchangeRouter.swapExactTokensForTokens(token1Amount, 0, path_TokentoBase, address(this), block.timestamp)[1];
        // }
        // if(_baseToken != _token0){
            path_TokentoBase[0] = token0;
            path_TokentoBase[1] = baseToken;
            token0Amount = exchangeRouter.swapExactTokensForTokens(token1Amount.add(token0Amount), 0, path_TokentoBase, address(this), block.timestamp)[1];
        // }
        IERC20(baseToken).transfer(msg.sender, token0Amount);
    }
    
    function calcOptimalSwapAmt(uint res, uint amt) internal pure returns (uint swapAmount){
        //0.2% swap fee and 0.02% deducted as protocol fee
        //(sqrt(res.mul((res.mul((2-f)^2)).add(amt.mul(4*(1-h)*(1-f))))).sub(res.mul(2-f))).div(2*(1-f)*(1-h));
        swapAmount = (sqrt(res.mul((res.mul(399200400)).add(amt.mul(399120160)))).sub(res.mul(19980))).div(19956);
    }
    ...
    function exit(uint256 amount) external override nonReentrant{
        ...
        if( status ) {
            rewardPool.withdraw(pid, mv, false);   
        }
    ...
    function reinvest() public nonReentrant{
        ...
        //Swap an optimum amount of WEX to USDT
        address[] memory path_RewardToToken = new address[](2);
        path_RewardToToken[0] = rewardToken;
        path_RewardToToken[1] = token0;
        ( , uint resT1, ) = lPToken.getReserves();
        uint swapAmount = calcOptimalSwapAmt(resT1, reward);
        uint token1Amount = reward.sub(swapAmount);
    ...
    function getStrategyTokenPrice() public view override returns (uint) {
        ...
        path_TokentoStableCoin[0] = token1;
        path_TokentoStableCoin[1] = token0;
        uint p1_0 = exchangeRouter.getAmountsOut(1e18, path_TokentoStableCoin)[1];
        (path_TokentoStableCoin[0], path_TokentoStableCoin[1]) = (token0, stableCoin);
        uint p0 = exchangeRouter.getAmountsOut(1e18, path_TokentoStableCoin)[1]; 
        ...
        uint sqrtP = (sqrt(p1_0.mul(1e18))).mul(p0).div(1e18);
        // else{
