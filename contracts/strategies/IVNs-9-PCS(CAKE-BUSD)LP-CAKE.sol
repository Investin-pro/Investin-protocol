...
contract InvestinStrategyToken is ERC20("PCS(CAKE-BUSD)LP-CAKE", "IVNs-9"), Ownable, ReentrancyGuard, iStrategy {//3213k 3172k 
    ...
    iExchangePair constant lPToken = iExchangePair(0x804678fa97d91B974ec2af3c843270886528a9E6);
    address constant token0 = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
  ...
  constructor(address[] memory _fundFactories) public {
    ...
    function enter(uint256 amount, bool isFund) external override nonReentrant{
        ...
        //if(_baseToken!=_token0){
            address[] memory path_BaseToToken0 = new address[](2); // path_BaseToToken0
    ...
    function reinvest() public nonReentrant{
        ...
        //Here token0 is CAKE itself, lets swap an aoptimum amount of CAKE to token1 i.e. BUSD to provide liquidity 
        address[] memory path_RewardToToken1 = new address[](2);
        path_RewardToToken1[0] = rewardToken;
        path_RewardToToken1[1] = token1;
        (uint resT0, , ) = lPToken.getReserves();//Since CAKE == Token0 
        uint swapAmount = calcOptimalSwapAmt(resT0, reward);
        uint token0Amount = reward.sub(swapAmount);
        uint token1Amount = exchangeRouter.swapExactTokensForTokens(swapAmount, 0, path_RewardToToken1, address(this), block.timestamp)[1];
        
        
        // address[] memory path_Token1toToken0 = new address[](2);
        // path_Token1toToken0[0] = token1;
        // path_Token1toToken0[1] = token0;
        // uint token0Amount = exchangeRouter.swapExactTokensForTokens(swapAmount, 0, path_Token1toToken0, address(this), block.timestamp)[1];
    ...
    function getStrategyTokenPrice() public view override returns (uint) {
        ...
        address[] memory path_Token0toToken1 = new address[](2);
        path_Token0toToken1[0] = token0;
        path_Token0toToken1[1] = stableCoin;
        uint p0 = exchangeRouter.getAmountsOut(1e18, path_Token0toToken1)[1]; //_token0==_baseToken?1e18:getTokenPrice(_token0, _baseToken);
        uint p1 = 1e18;//_token1==_baseToken?1e18:getTokenPrice(_token1, _baseToken);
