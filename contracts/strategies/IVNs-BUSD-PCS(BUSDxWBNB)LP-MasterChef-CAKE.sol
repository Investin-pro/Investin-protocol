...
contract TESTINStrategyToken is ERC20("PCS(BUSD-WBNB)LP-CAKE", "UCBs-1"), Ownable, iStrategy {//3213k 3172k 
    ...
    function enter(uint256 amount, bool isFund) external override{
        ...
        if(isFund==false){
            uint _entryFee = (amount*entryFee)/1e8;
        ...
        //if(_baseToken!=_token0){
            ...
            uint swapAmount = ( sqrt( resT1 * ( ( 399000625 * resT1 ) + ( 399000000 * amount ) ) ) - ( 19975 * resT1 ) ) / 19995;
            token1Amount = amount - swapAmount;
    ...
    function exitHelper(uint256 mv) internal{
        ...
        // }
        IERC20(baseToken).transfer(msg.sender, token1Amount+token0Amount);
    }
     
    function exit(uint256 amount) external override{
        uint mv = ( amount * ( ( principalStaked * 1e18 ) / totalSupply() ) ) / 1e18;
        if( status ) {
    ...
    function reinvest() public{
        ...
        uint256 bounty = ( reward * reinvestIncentive ) / 1e8;
        IERC20(rewardToken).transfer(msg.sender, bounty);
        reward -= bounty;
        address[] memory path_RewardToToken0 = new address[](2);
        path_RewardToToken0[0] = rewardToken;
        path_RewardToToken0[1] = token0;
        uint amount = exchangeRouter.swapExactTokensForTokens(reward, 0, path_RewardToToken0, address(this), block.timestamp)[1];
        (uint resT0, , ) = lPToken.getReserves();
        uint swapAmount = ( sqrt( resT0 * ( ( 399000625 * resT0 ) + ( 399000000 * amount ) ) ) - ( 19975 * resT0 ) ) / 19995;
        uint token0Amount = amount - swapAmount;
        address[] memory path_Token0toToken1 = new address[](2);
        path_Token0toToken1[0] = token0;
        path_Token0toToken1[1] = token1;
        uint token1Amount = exchangeRouter.swapExactTokensForTokens(swapAmount, 0, path_Token0toToken1, address(this), block.timestamp)[1];
